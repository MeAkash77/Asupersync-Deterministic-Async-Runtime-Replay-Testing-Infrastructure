...
//!
//! Usage:
//!   rch exec -- env CARGO_TARGET_DIR=${TMPDIR:-/tmp}/rch_target_conformance_bin_docs cargo run --bin h2_goaway_conformance
//!   rch exec -- env CARGO_TARGET_DIR=${TMPDIR:-/tmp}/rch_target_conformance_bin_docs cargo run --bin h2_goaway_conformance -- --format json
//!   rch exec -- env CARGO_TARGET_DIR=${TMPDIR:-/tmp}/rch_target_conformance_bin_docs cargo run --bin h2_goaway_conformance -- --output report.md
...
#[command(about = "HTTP/2 GOAWAY frame conformance tester")]
struct Args {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut tester = asupersync_conformance::GoAwayConformanceTester::new();
...
fn generate_summary_output(report: &GoAwayComplianceReport) -> String {
    let mut output = String::new();

    output.push_str("HTTP/2 GOAWAY FRAME CONFORMANCE SUMMARY\n");
    output.push_str("======================================\n\n");
...
fn print_test_summary(report: &GoAwayComplianceReport) {
    eprintln!("╭─ HTTP/2 GOAWAY CONFORMANCE RESULTS ─╮");
    eprintln!("│                                      │");
...
fn has_incomplete_coverage(report: &GoAwayComplianceReport) -> bool {
    report.total_cases == 0 || report.summary.skipped > 0 || report.summary.expected_failures > 0
}

fn exit_code(report: &GoAwayComplianceReport) -> i32 {
...
mod tests {
    ...
    fn synthetic_report(
        ...
        skipped: usize,
    ) -> GoAwayComplianceReport {
        GoAwayComplianceReport {
            test_run_id: "synthetic".to_string(),
            timestamp: chrono::Utc::now(),
            total_cases,
            results: Vec::new(),
            summary: GoAwayComplianceSummary {
                total_cases,
                passed: total_cases

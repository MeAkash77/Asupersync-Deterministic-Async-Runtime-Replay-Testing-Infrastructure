//! HTTP/2 DATA Frame END_STREAM Conformance Test Runner
//!
//! Runs HTTP/2 DATA END_STREAM checks against RFC expected states while the h2
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("🔧 HTTP/2 DATA Frame END_STREAM Conformance Tester");
    println!("   {}", reference_scope_line());
    println!("   Focus: Stream state transitions, END_STREAM semantics");
    println!();

    // Create and configure the tester
    let mut tester = asupersync_conformance::DataEndStreamConformanceTester::new();
...
fn generate_summary_output(report: &DataEndStreamComplianceReport) -> String {
    ...
    // Summary of stream state behavior

...
- proof that docs classify `MessageChannel`, `MessagePort`, and
  `BroadcastChannel` as guarded public browser application-boundary helpers
- proof that docs classify `ReadableStream` / `WritableStream` public helpers as browser-native byte wrappers while Rust `AsyncRead` / `AsyncWrite` browser-core ABI parity remains substrate-only
- the focused reactor/stream test output for MessagePort/BroadcastChannel and
  WHATWG stream validation paths
...
1. If the goal is direct off-main-thread Browser Edition execution, bootstrap
   ...
   messaging or stream helper as a runtime lane.
...
3. If the goal is browser-native byte streaming, use `BrowserReadableStream`
   or `BrowserWritableStream` only after explicit `BrowserNativeStreamCapability`
   support passes, and keep Rust `AsyncRead` / `AsyncWrite` wasm ABI claims out
   of user-facing docs until a separate proof bead ships that ABI.
4. If the hop crosses into server, edge, Node, or another non-browser
   boundary, route through explicit bridge-only adapters instead of widening
   the browser direct-runtime contract.
5. If WebTransport is unavailable or handshake/datagram setup fails, fall back
...
| Browser-native messaging (`MessageChannel`, `MessagePort`, `BroadcastChannel`) | `guarded canary-only` | explicit capability diagnostics pass, package contracts stay green, and docs keep the helpers at the browser application boundary | any claim that these helpers are asupersync structured channels, raw transports, cross-origin bridges, or direct runtime lanes |
| Browser-native WHATWG streams (`ReadableStream`, `WritableStream`) | `guarded canary-only` | explicit capability diagnostics pass, byte/lifecycle contracts stay green, and docs keep Rust `AsyncRead` / `AsyncWrite` browser-core ABI parity substrate-only | any claim that public JS/TS wrappers exercise the Rust I/O ABI or provide raw transport/filesystem/process parity |

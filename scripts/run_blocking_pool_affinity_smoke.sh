...
ARTIFACT="${PROJECT_ROOT}/artifacts/blocking_pool_affinity_smoke_contract_v1.json"
MODE="execute"
SCENARIO=""
LIST_ONLY=0
OUTPUT_ROOT_OVERRIDE="${BLOCKING_POOL_AFFINITY_SMOKE_OUTPUT_DIR:-}"
ARTIFACT_ROOT_OVERRIDE="${BLOCKING_POOL_AFFINITY_SMOKE_ARTIFACT_ROOT:-}"
RUN_ID_OVERRIDE="${BLOCKING_POOL_AFFINITY_SMOKE_RUN_ID:-}"
RCH_BIN="${RCH_BIN:-$HOME/.local/bin/rch}"
RCH_LOCAL_FALLBACK_PATTERN='^\[RCH\] local \(|falling back to local|local fallback|fallback to local|executing locally'

usage() {
    cat <<'USAGE'
Usage: ./scripts/run_blocking_pool_affinity_smoke.sh [options]
...
Options:
  ...
  --dry-run               Emit manifests without executing the affinity proof
  --execute               Execute the affinity proof (default)
  -h, --help              Show help
USAGE
}
...
require_tools() {
    if ! command -v jq >/dev/null 2>&1; then
        echo "FATAL: jq is required for blocking-pool affinity smoke runner" >&2
        exit 1
    fi
...
list_scenarios() {
    echo "=== Blocking Pool Affinity Smoke Scenarios ==="
...
extract_report_from_log() {
    ...
    mkdir -p "$output_dir"
    awk '
        /BLOCKING_POOL_AFFINITY_REPORT_JSON_BEGIN/ { capture=1; next }
        /BLOCKING_POOL_AFFINITY_REPORT_JSON_END/ { capture=0; exit }
        capture { print }
...
REPORT_ARTIFACT_ROOT="${ARTIFACT_ROOT_OVERRIDE:-.blocking-pool-affinity-smoke-artifacts}"
...
SCENARIO_REPORT_PATH="${REPORT_ARTIFACT_DIR}/blocking_pool_affinity_report.json"
...
STATUS="dry_run"
MESSAGE="dry-run only; expected projection emitted without executing the blocking-pool affinity proof"
...
if [ "$MODE" = "execute" ]; then
    ...
    while kill -0 "$COMMAND_PID" 2>/dev/null; do
        if grep -q 'BLOCKING_POOL_AFFINITY_REPORT_JSON_END' "$RUN_LOG_PATH" 2>/dev/null \
        ...
        if [ "$POLL_SECONDS" -ge "$MAX_POLL_SECONDS" ]; then
            ...
            printf 'FATAL: timed out waiting for blocking-pool affinity smoke proof markers\n' >>"$RUN_LOG_PATH"
            break
        fi
    done
    ...
    if [ "$COMMAND_EXIT_CODE" -eq 0 ] && extract_report_from_log "$RUN_LOG_PATH" "$SCENARIO_REPORT_PATH"; then
        ...
        if [ "$COMMAND_EXIT_CODE" -eq 86 ]; then
            ...
            MESSAGE="blocking-pool affinity proof failed"
        fi
    fi
else
...
if [ -f "$SCENARIO_REPORT_PATH" ]; then
    printf 'blocking_pool_affinity_report=%s\n' "$SCENARIO_REPORT_PATH"
fi

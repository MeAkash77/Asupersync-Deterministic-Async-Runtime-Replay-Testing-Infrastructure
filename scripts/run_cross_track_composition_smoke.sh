...
# - cross-track-composition-smoke-bundle-v1
# - cross-track-composition-smoke-run-report-v1
...
run_scenario() {
    ...
    local -a command=(
        ...
        "CARGO_TARGET_DIR=${TMPDIR:-/tmp}/rch_target_cross_track_composition_${safe_sid}"
        "${CARGO_BIN:-cargo}" test -p asupersync --test cross_track_composition_contract --features test-internals
    )

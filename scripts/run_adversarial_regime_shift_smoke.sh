...
run_scenario() {
    ...
    local -a command=(
        ...
        "CARGO_TARGET_DIR=${TMPDIR:-/tmp}/rch_target_adversarial_regime_shift_${safe_sid}"
        "${CARGO_BIN:-cargo}" test -p asupersync --test adversarial_regime_shift_contract --features test-internals
    )

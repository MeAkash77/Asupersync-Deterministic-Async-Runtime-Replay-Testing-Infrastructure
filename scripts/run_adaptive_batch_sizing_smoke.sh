...
ARTIFACT="${PROJECT_ROOT}/artifacts/adaptive_batch_sizing_smoke_contract_v1.json"

LIST_ONLY=0
MODE="dry-run"
SCENARIO=""
OUTPUT_ROOT_OVERRIDE="${ADAPTIVE_BATCH_SIZING_SMOKE_OUTPUT_DIR:-}"
ARTIFACT_ROOT_OVERRIDE="${ADAPTIVE_BATCH_SIZING_SMOKE_ARTIFACT_ROOT:-}"
RUN_ID_OVERRIDE="${ADAPTIVE_BATCH_SIZING_SMOKE_RUN_ID:-}"
RCH_BIN="${RCH_BIN:-$HOME/.local/bin/rch}"
RCH_LOCAL_FALLBACK_PATTERN='^\[RCH\] local \(|falling back to local|local fallback|fallback to local|executing locally'

usage() {
    cat <<'EOF'
Usage: ./scripts/run_adaptive_batch_sizing_smoke.sh [options]
...
extract_report_from_log() {
    ...
    mkdir -p "$output_dir"
    awk '
        /ADAPTIVE_BATCH_SIZING_REPORT_JSON_BEGIN/ { capture=1; next }
        /ADAPTIVE_BATCH_SIZING_REPORT_JSON_END/ { capture=0; exit }
        capture { print }
...
ARTIFACT_ROOT="${ARTIFACT_ROOT_OVERRIDE:-${PROJECT_ROOT}/.adaptive-batch-sizing-smoke-artifacts/run_${RUN_ID}/${SCENARIO}}"
RUN_LOG_PATH="${RUN_DIR}/run.log"
BUNDLE_MANIFEST_PATH="${RUN_DIR}/bundle_manifest.json"
RUN_REPORT_PATH="${RUN_DIR}/run_report.json"
SCENARIO_REPORT_PATH="${ARTIFACT_ROOT}/adaptive_batch_sizing_report.json"
RCH_TAIL_TIMEOUT_SECONDS="${ADAPTIVE_BATCH_SIZING_RCH_TIMEOUT_SECONDS:-300}"

mkdir -p "$RUN_DIR"
HOST_FINGERPRINT_JSON="$(host_fingerprint_json)"
STARTED_TS="$(date -u +%Y-%m-%dT%H:%M:%SZ)"

RCH_TARGET_DIR="${TMPDIR:-/tmp}/rch_target_adaptive_batch_sizing"
COMMAND_ARGS=(
    "$RCH_BIN"
    exec
    --
    ...
    "ASUPERSYNC_ADAPTIVE_BATCH_SIZING_CONTRACT_PATH=${ARTIFACT}"
    "ASUPERSYNC_ADAPTIVE_BATCH_SIZING_SCENARIO=${SCENARIO}"
    "ASUPERSYNC_ADAPTIVE_BATCH_SIZING_REPORT_PATH=${SCENARIO_REPORT_PATH}"
    "${CARGO_BIN:-cargo}"
    test
    -p
    asupersync
    --test
    adaptive_batch_sizing_contract
    adaptive_batch_sizing_smoke_contract_emits_report
    --features
    test-internals
    --
    --nocapture
...
if [ "$MODE" = "dry-run" ]; then
    ...
    while kill -0 "$COMMAND_PID" 2>/dev/null; do
        if grep -q 'ADAPTIVE_BATCH_SIZING_REPORT_JSON_END' "$RUN_LOG_PATH" 2>/dev/null \
        ...
        if [ "$POLL_SECONDS" -ge "$MAX_POLL_SECONDS" ]; then
            ...
            printf 'FATAL: timed out waiting for adaptive batch sizing proof markers\n' >>"$RUN_LOG_PATH"
            break
        fi
    done
    ...
    if [ "$COMMAND_EXIT_CODE" -ne 86 ] && grep -Eiq "$RCH_LOCAL_FALLBACK_PATTERN" "$RUN_LOG_PATH" 2>/dev/null; then
        COMMAND_EXIT_CODE=86
        printf 'FATAL: rch local fallback detected; refusing local cargo execution\n' >>"$RUN_LOG_PATH"
    fi
    ...
    if [ "$STATUS" = "passed" ]; then
        if ! extract_report_from_log "$RUN_LOG_PATH" "$SCENARIO_REPORT_PATH"; then
            SCRIPT_EXIT_CODE=1
            STATUS="failed"
            MESSAGE="adaptive batch sizing report JSON markers missing from run.log"
        else
...
jq -n \
    --arg schema_version "adaptive-batch-sizing-smoke-bundle-v1" \
...
jq -n \
    --arg schema_version "adaptive-batch-sizing-run-report-v1" \
    --arg scenario_id "$SCENARIO" \
    --arg status "$STATUS" \
    --arg message "$MESSAGE" \
    --arg mode "$MODE" \

...
ARTIFACT="${PROJECT_ROOT}/artifacts/governor_state_snapshot_equivalence_smoke_contract_v1.json"
MODE="execute"
SCENARIO=""
LIST_ONLY=0
OUTPUT_ROOT_OVERRIDE="${GOVERNOR_STATE_SNAPSHOT_SMOKE_OUTPUT_DIR:-}"
ARTIFACT_ROOT_OVERRIDE="${GOVERNOR_STATE_SNAPSHOT_SMOKE_ARTIFACT_ROOT:-}"
RUN_ID_OVERRIDE="${GOVERNOR_STATE_SNAPSHOT_SMOKE_RUN_ID:-}"
RCH_BIN="${RCH_BIN:-$HOME/.local/bin/rch}"

usage() {
    cat <<'USAGE'
Usage: ./scripts/run_governor_state_snapshot_smoke.sh [options]
...
Options:
  ...
  --dry-run               Emit manifests without executing the governor snapshot proof
  --execute               Execute the governor snapshot proof (default)
  -h, --help              Show help
USAGE
}
...
require_tools() {
    if ! command -v jq >/dev/null 2>&1; then
        echo "FATAL: jq is required for governor snapshot smoke runner" >&2
        exit 1
    fi
...
list_scenarios() {
    echo "=== Governor State Snapshot Smoke Scenarios ==="
...
extract_report_from_log() {
    ...
    mkdir -p "$output_dir"
    awk '
        /GOVERNOR_STATE_SNAPSHOT_REPORT_JSON_BEGIN/ { capture=1; next }
        /GOVERNOR_STATE_SNAPSHOT_REPORT_JSON_END/ { capture=0; exit }
        capture { print }
...
REPORT_ARTIFACT_ROOT="${ARTIFACT_ROOT_OVERRIDE:-.governor-state-snapshot-smoke-artifacts}"
...
SCENARIO_REPORT_PATH="${REPORT_ARTIFACT_DIR}/governor_state_snapshot_report.json"
...
COMMAND_ARGS=(
    ...
    asupersync
    --lib
    governor_state_snapshot_smoke_contract_emits_report
    --features
    test-internals
    --
    --nocapture
...
STATUS="dry_run"
MESSAGE="dry-run only; expected projection emitted without executing the governor snapshot proof"
...
if [ "$MODE" = "execute" ]; then
    ...
    while kill -0 "$COMMAND_PID" 2>/dev/null; do
        if grep -q 'GOVERNOR_STATE_SNAPSHOT_REPORT_JSON_END' "$RUN_LOG_PATH" 2>/dev/null \
        ...
        if [ "$POLL_SECONDS" -ge "$MAX_POLL_SECONDS" ]; then
            ...
            printf 'FATAL: timed out waiting for governor snapshot proof markers\n' >>"$RUN_LOG_PATH"
            break
        fi
    done
    ...
    if [ "$COMMAND_EXIT_CODE" -eq 0 ] && extract_report_from_log "$RUN_LOG_PATH" "$SCENARIO_REPORT_PATH"; then
        ...
        STATUS="passed"
        MESSAGE="governor snapshot equivalence proof passed and emitted full-scan versus summary evidence"
    else
        ...
        if [ "$COMMAND_EXIT_CODE" -eq 86 ]; then
            ...
            MESSAGE="governor snapshot equivalence proof failed"
        fi
    fi
else

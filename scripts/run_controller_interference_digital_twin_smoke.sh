...
COMMAND_ARGS=(
    ...
    asupersync
    --test
    controller_interference_digital_twin_contract
    controller_interference_digital_twin_smoke_emits_report
    --features
    test-internals
    --
    --nocapture
...
MESSAGE="controller-interference digital-twin runner completed"
...
if [[ "$MODE" == "execute" ]]; then
    ...
    if [[ "$STATUS" != "failed" && "$COMMAND_STATUS" -ne 0 && "$REMOTE_TEST_PASSED" != "true" ]]; then
        ...
        MESSAGE="controller-interference digital-twin command failed with status ${COMMAND_STATUS}"
        echo "FATAL: ${MESSAGE}" >>"$RUN_LOG_PATH"
    fi
    if [[ "$STATUS" != "failed" && ! -s "$REPORT_PATH" ]]; then
        STATUS="failed"
        SCRIPT_EXIT_CODE=1
        MESSAGE="controller-interference digital-twin report missing after ${REPORT_SOURCE}"
        echo "FATAL: ${MESSAGE}" >>"$RUN_LOG_PATH"
    fi
    ...
    ' "$REPORT_PATH" >/dev/null; then
        STATUS="failed"
        SCRIPT_EXIT_CODE=1
        MESSAGE="controller-interference digital-twin report failed contract validation"
    fi
fi

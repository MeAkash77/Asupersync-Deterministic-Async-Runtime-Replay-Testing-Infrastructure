...
ARTIFACT="${PROJECT_ROOT}/artifacts/replay_minimization_validation_contract_v1.json"
...
case "$SCENARIO" in
  AA06-VALID-CANONICALIZATION)
    TEST_FILTER="canonical"
    ;;
  AA06-VALID-NORMALIZATION)
    TEST_FILTER="normalization"
    ;;
  AA06-VALID-RACE-DETECTION)
    TEST_FILTER="race"
    ;;
  AA06-VALID-CRASHPACK)
    TEST_FILTER="crashpack"
    ;;
  AA06-VALID-DIVERGENCE)
    TEST_FILTER="divergence"
    ;;
  *)
...
COMMAND_ARGS=(
  ...
  "CARGO_TARGET_DIR=${TMPDIR:-/tmp}/rch_target_replay_minimization_validation"
  "${CARGO_BIN:-cargo}"
  test
  -p
  asupersync
...
STATUS="passed"
MESSAGE="replay minimization validation smoke passed"
if grep -Eiq "$RCH_LOCAL_FALLBACK_PATTERN" "$RUN_LOG"; then
  ...
  STATUS="failed"
  MESSAGE="replay minimization validation smoke failed"
fi

...
ARTIFACT="${PROJECT_ROOT}/artifacts/runtime_latency_budget_certificate_v1.json"
OUTPUT_ROOT="${LATENCY_BUDGET_CERTIFICATE_OUTPUT_ROOT:-${PROJECT_ROOT}/target/latency-budget-certificate-smoke}"
RUN_ID="${LATENCY_BUDGET_CERTIFICATE_RUN_ID:-$(date +%Y%m%d_%H%M%S)}"
MODE="execute"
LIST_ONLY=0
...
usage() {
    cat <<'USAGE'
Usage: bash scripts/run_latency_budget_certificate_smoke.sh [options]

Options:
  --list                  List latency-budget certificate smoke scenarios and exit
...
list_scenarios() {
    jq -r '.smoke_scenarios[] | "  \(.scenario_id) / \(.certificate_id): \(.expected_verdict)"' "$ARTIFACT"
}
...
extract_report_from_log() {
    ...
    local report_path="$2"
    awk '
        /LATENCY_BUDGET_CERTIFICATE_REPORT_JSON_BEGIN/ { capture=1; next }
        /LATENCY_BUDGET_CERTIFICATE_REPORT_JSON_END/ { capture=0; exit }
        capture { print }
...
validate_smoke_report() {
    ...
    jq -e '
        ...
        and .contract_version == "runtime-latency-budget-certificate-v1"
        and .bead_id == "asupersync-d87ytw.2"
        and .status == "passed"
        and (.rows | length) == 3
        and ([.rows[].verdict] | sort == ["fail_closed", "no_win", "pass"])
        and all(.rows[];
            has("certificate_id")
            and has("certificate_hash")
            and has("scenario_id")
            and has("candidate_id")
            and has("fallback_profile")
            and has("taxonomy_version")
            and has("p50_latency_ns")
            and has("p95_latency_ns")
            and has("p99_latency_ns")
            and has("p999_latency_ns")
            and has("budget_p999_latency_ns")
            and has("term_breakdown")
            and has("uncertainty_interval")
            and has("sample_count")
            and has("unknown_residual_ns")
            and has("unknown_residual_basis_points")
...
SMOKE_REPORT_PATH="${RUN_DIR}/latency_budget_certificate_report.json"
...
COMMAND_ARGS=(
    ...
    asupersync
    --test
    runtime_latency_budget_certificate_contract
    latency_budget_certificate_smoke_emits_report
    --features
    test-internals
    --
    --nocapture
...
COMMAND_EXIT_CODE=0
SCRIPT_EXIT_CODE=0
MESSAGE="latency-budget certificate smoke completed"
...
if [ "$MODE" = "dry-run" ]; then
    ...
    if grep -Eiq "$RCH_LOCAL_FALLBACK_PATTERN" "$RUN_LOG_PATH"; then
        ...
        if extract_report_from_log "$RUN_LOG_PATH" "$SMOKE_REPORT_PATH" && validate_smoke_report "$SMOKE_REPORT_PATH"; then
            ...
            MESSAGE="rch proof command failed before a valid latency-budget report was emitted"
        fi
        ...
        COMMAND_EXIT_CODE=1
        SCRIPT_EXIT_CODE=1
        MESSAGE="latency-budget report markers missing from run log"
    elif ! validate_smoke_report "$SMOKE_REPORT_PATH"; then
        STATUS="failed"
        VALIDATION_PASSED=false
        COMMAND_EXIT_CODE=1
        SCRIPT_EXIT_CODE=1
        MESSAGE="smoke report missing required certificate logging fields"
    fi
fi
...
echo "LATENCY_BUDGET_CERTIFICATE_SMOKE_SUMMARY"

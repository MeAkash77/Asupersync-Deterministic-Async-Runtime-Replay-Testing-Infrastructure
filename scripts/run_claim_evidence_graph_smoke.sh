...
run_scenario() {
    ...
    local -a command=(
        ...
        "CARGO_TARGET_DIR=${TMPDIR:-/tmp}/rch_target_claim_evidence_graph_${safe_sid}"
        "${CARGO_BIN:-cargo}" test -p asupersync --test claim_evidence_graph_contract --features test-internals
    )

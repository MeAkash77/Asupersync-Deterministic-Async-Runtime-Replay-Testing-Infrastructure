...
ARTIFACT="${PROJECT_ROOT}/artifacts/otlp_brownout_shedding_smoke_contract_v1.json"
...
usage() {
    cat <<'EOF'
Usage: ./scripts/run_otlp_brownout_shedding_smoke.sh [options]
...
extract_report_from_log() {
    ...
    mkdir -p "$output_dir"
    awk '
        /OTLP_BROWNOUT_SHEDDING_REPORT_JSON_BEGIN/ { capture=1; next }
        /OTLP_BROWNOUT_SHEDDING_REPORT_JSON_END/ { capture=0; exit }
        capture { print }
...
ARTIFACT_ROOT="${ARTIFACT_ROOT_OVERRIDE:-${PROJECT_ROOT}/.otlp-brownout-shedding-smoke-artifacts/run_${RUN_ID}/${SCENARIO}}"
RUN_LOG_PATH="${RUN_DIR}/run.log"
BUNDLE_MANIFEST_PATH="${RUN_DIR}/bundle_manifest.json"
RUN_REPORT_PATH="${RUN_DIR}/run_report.json"
SCENARIO_REPORT_PATH="${ARTIFACT_ROOT}/otlp_brownout_shedding_report.json"
...
if [ "$MODE" = "dry-run" ]; then
    ...
    if grep -Eiq "$RCH_LOCAL_FALLBACK_PATTERN" "$RUN_LOG_PATH"; then
        ...
        if ! extract_report_from_log "$RUN_LOG_PATH" "$SCENARIO_REPORT_PATH"; then
            SCRIPT_EXIT_CODE=1
            STATUS="failed"
            MESSAGE="OTLP brownout report JSON markers missing from run.log"
        elif [ "$EXPECTED_REPORT_PROJECTION_JSON" != "null" ] && ! jq -en \
            --argjson expected "$EXPECTED_REPORT_PROJECTION_JSON" \
            --argjson actual "$(jq -c '.report_projection' "$SCENARIO_REPORT_PATH")" \
...
jq -n \
    --arg schema_version "otlp-brownout-shedding-smoke-bundle-v1" \
...
jq -n \
    --arg schema_version "otlp-brownout-shedding-smoke-run-report-v1" \
    --arg scenario_id "$SCENARIO" \
    --arg mode "$MODE" \
    --arg status "$STATUS" \
    --arg message "$MESSAGE" \
...
===================================================================
   OTLP BROWNOUT SHEDDING SMOKE SUMMARY                            
===================================================================
  Run dir:   ${RUN_DIR}
  Mode:      ${MODE^^}
  Status:    ${STATUS}

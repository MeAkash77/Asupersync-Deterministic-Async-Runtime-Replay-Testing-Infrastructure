...
run_scenario() {
    ...
    local -a command=(
        ...
        "CARGO_TARGET_DIR=${TMPDIR:-/tmp}/rch_target_controller_sandbox_${safe_sid}"
        "${CARGO_BIN:-cargo}" test -p asupersync --test controller_sandbox_contract --features test-internals
    )

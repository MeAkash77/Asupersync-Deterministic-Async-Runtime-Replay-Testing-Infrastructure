...
ARTIFACT="${PROJECT_ROOT}/artifacts/hindsight_logging_nondeterminism_capture_v1.json"
...
case "$SCENARIO" in
  AA06-SMOKE-NONDETERMINISM-CATALOG)
    TEST_FILTER="nondeterminism_source_catalog"
    ;;
  AA06-SMOKE-DERIVED-STATE-EXCLUSION)
    TEST_FILTER="excluded_derived"
    ;;
  AA06-SMOKE-SIZE-BUDGET)
    TEST_FILTER="size_budget"
    ;;
  AA06-SMOKE-REPLAY-FORMAT)
    TEST_FILTER="replay_format"
    ;;
  *)
...
COMMAND_ARGS=(
  ...
  "CARGO_TARGET_DIR=${TMPDIR:-/tmp}/rch_target_hindsight_logging"
  "${CARGO_BIN:-cargo}"
  test
  -p
  asupersync
  --test
  hindsight_logging_nondeterminism_capture_contract
  --features
  test-internals
  "$TEST_FILTER"
  --
...
STATUS="passed"
MESSAGE="hindsight logging smoke passed"
if grep -Eiq "$RCH_LOCAL_FALLBACK_PATTERN" "$RUN_LOG"; then
  ...
  STATUS="failed"
  MESSAGE="hindsight logging smoke failed"
fi

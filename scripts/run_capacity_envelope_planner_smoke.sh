...
ARTIFACT="${PROJECT_ROOT}/artifacts/capacity_envelope_planner_smoke_contract_v1.json"
...
usage() {
    cat <<'EOF'
Usage: ./scripts/run_capacity_envelope_planner_smoke.sh [options]
...
ARTIFACT_ROOT="${ARTIFACT_ROOT_OVERRIDE:-${PROJECT_ROOT}/.capacity-envelope-planner-smoke-artifacts/run_${RUN_ID}/${SCENARIO}}"
RUN_LOG_PATH="${RUN_DIR}/run.log"
BUNDLE_MANIFEST_PATH="${RUN_DIR}/bundle_manifest.json"
RUN_REPORT_PATH="${RUN_DIR}/run_report.json"
SCENARIO_REPORT_PATH="${ARTIFACT_ROOT}/capacity_envelope_certificate.json"
RCH_TAIL_TIMEOUT_SECONDS="${CAPACITY_ENVELOPE_RCH_TIMEOUT_SECONDS:-300}"
...
COMMAND_ARGS=(
    ...
    "CARGO_TARGET_DIR=${TMPDIR:-/tmp}/rch_target_capacity_envelope_planner"
    "ASUPERSYNC_CAPACITY_ENVELOPE_CONTRACT_PATH=${ARTIFACT}"
    "ASUPERSYNC_CAPACITY_ENVELOPE_SCENARIO=${SCENARIO}"
    "ASUPERSYNC_CAPACITY_ENVELOPE_REPORT_PATH=${SCENARIO_REPORT_PATH}"
    "${CARGO_BIN:-cargo}"
    test
    -p
    asupersync
    --test
    capacity_envelope_planner_contract
    capacity_envelope_smoke_contract_emits_report
    --features
    test-internals
    --
    --nocapture
...
jq -n \
    --arg schema_version "capacity-envelope-planner-smoke-bundle-v1" \
...
jq -n \
    --arg schema_version "capacity-envelope-planner-run-report-v1" \
    --arg scenario_id "$SCENARIO" \
    --arg status "$STATUS" \
    --arg message "$MESSAGE" \
    --arg mode "$MODE" \

...
TEST_FILTER="${1:-}"
RCH_BIN="${RCH_BIN:-rch}"
RCH_TARGET_DIR="${RCH_TARGET_DIR:-${TMPDIR:-/tmp}/rch-target-messaging-e2e-${USER:-unknown}-${TIMESTAMP}-$$}"
...
if ! run_cargo check --test e2e_messaging --all-features >"${ARTIFACT_DIR}/compile_errors.log" 2>&1; then
